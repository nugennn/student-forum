...
# views.py -> questionDetailView (Corrected code)

    getDat = data.history.first()
    # Check if a previous record exists before trying to access it
    if getDat and getDat.prev_record:
        getLastRecord = getDat.prev_record
    else:
        # If there's no history, or only one record, set it to None
        getLastRecord = None
